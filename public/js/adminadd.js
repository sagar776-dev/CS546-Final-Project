(function ($) {
  ...
  function alphabetsStringValidation(input, value) {
    if (!value) {
      ...
      value = value.trim();
      if (value.length < 1) errorList.push(`${input} should not be empty`);
  ...
  function validateReleaseDate(date) {
    if (!date) {
      errorList.push("You must provide a release date for your Product");
    } else {
      if (typeof date !== "string") errorList.push("runtime must be a string");
      else if (date.trim().length === 0)
        errorList.push(
          "date cannot be an empty string or string with just spaces"
        );
    }
    return date.trim();
  }
  ...
  function inputValidation(product) {
    if (!product.name) errorList.push("Product Name should not be empty");
    else {
    ...
    if (!product.price) {
      ...
      if (product.price.length < 1) errorList.push("Price should not be empty");
      else if (isNaN(product.price)) errorList.push("Price should be a number");
    ...
    if (!product.url) {
      ...
      if (product.url.length < 1) errorList.push("URL should not be empty");
    ...
    if (product.details.length < 1) {
      ...
      product.details.find((element) => element.name === "Screen Size").value =
        alphanumericStringValidation(
          "Screen Size",
          (
            product.details.find((element) => element.name === "Screen Size") ||
            emptyDetailsObj
          ).value
        );
      product.details.find(
        (element) => element.name === "Processor Model"
      ).value = alphanumericStringValidation(
        "Processor Model",
        (
          product.details.find(
            (element) => element.name === "Processor Model"
      ...
      product.details.find(
        (element) => element.name === "Screen Resolution"
      ).value = alphanumericStringValidation(
        "Screen Resolution",
        (
          product.details.find(
      ...
      product.details.find(
        (element) => element.name === "Operating System"
      ).value = alphanumericStringValidation(
        "Operating System",
        (
          product.details.find(
      ...
      if (product.category === "laptops") {
        product.details.find(
          (element) => element.name === "System Memory (RAM)"
        ).value = alphanumericStringValidation(
          "System Memory (RAM)",
          (
            product.details.find(
        ...
        product.details.find((element) => element.name === "Graphics").value =
          alphanumericStringValidation(
            "Graphics",
            (
              product.details.find((element) => element.name === "Graphics") ||
              emptyDetailsObj
            ).value
          );
        product.details.find(
          (element) => element.name === "Storage Type"
        ).value = alphanumericStringValidation(
          "Storage Type",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Total Storage Capacity"
        ).value = alphanumericStringValidation(
        ...
        product.details.find(
          (element) => element.name === "Touch Screen"
        ).value = alphanumericStringValidation(
          "Touch Screen",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Processor Model Number"
        ).value = alphanumericStringValidation(
        ...
        product.details.find(
          (element) => element.name === "Battery Type"
        ).value = alphanumericStringValidation(
          "Battery Type",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Backlit Keyboard"
        ).value = alphabetsStringValidation(
          "Backlit Keyboard",
          (
            product.details.find(
              (element) => element.name === "Backlit Keyboard"
        ...
        product.details.find((element) => element.name === "Brand").value =
          alphanumericStringValidation(
            "Brand",
            (
              product.details.find((element) => element.name === "Brand") ||
              emptyDetailsObj
            ).value
          );
        product.details.find(
          (element) => element.name === "Model Number"
        ).value = alphanumericStringValidation(
          "Model Number",
          (
            product.details.find(
        ...
        product.details.find((element) => element.name === "Carrier").value =
          alphabetsStringValidation(
            "Carrier",
            (
              product.details.find((element) => element.name === "Carrier") ||
              emptyDetailsObj
            ).value
          );
        product.details.find(
          (element) => element.name === "Wireless Technology"
        ).value = alphanumericStringValidation(
          "Wireless Technology",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Voice Assistant Built-in"
        ).value = alphabetsStringValidation(
        ...
        product.details.find(
          (element) => element.name === "Bluetooth Enabled"
        ).value = alphabetsStringValidation(
          "Bluetooth Enabled",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Keyboard Type"
        ).value = alphabetsStringValidation(
          "Keyboard Type",
          (
            product.details.find(
        ...
        product.details.find(
          (element) => element.name === "Wireless Compatibility"
        ).value = alphabetsStringValidation(
        ...
        product.details.find(
          (element) => element.name === "Wireless Connectivity"
        ).value = alphanumericStringValidation(
